/**
 * 自动生成的API接口实现
 * 生成时间: Thu Oct 16 15:39:57 CST 2025
 */

import type { SdkStream, SdkRequestOptions } from 'sdkwork-commons-typescript';
import type {
  ChatCompletionParam
} from 'sdkwork-sdk-api-typescript';

import type {
  ChatCompletion,
  ChatCompletionChunk
} from 'sdkwork-sdk-api-typescript';

import { ChatManager } from 'sdkwork-sdk-manager-typescript';
import { ManagerFactory } from "@/services/src/factory";
/**
 * Chat API接口实现
 */
export class ChatService {
   chatManager: ChatManager;
   constructor() {
      this.chatManager = ManagerFactory.create( ChatManager );
   }

  /**
   * Stop a chat completion stream
   */
  async stop(queryParams?: { conversationId: string }, options?: SdkRequestOptions): Promise<Boolean> {
    const response = await this.chatManager.stop(queryParams, options);
    if(response == null || response.data == null){
      return Promise.reject(new Error("data error!"));
    }
    return response.data as Boolean;
  }


  /**
   * Create a chat completion
   */
  async create(data: ChatCompletionParam, queryParams?: { conversationId?: string }, options?: SdkRequestOptions): Promise<ChatCompletion | SdkStream<ChatCompletionChunk>> {
    const response = await this.chatManager.create(data, queryParams, options);
    if ( data.stream ) {
        return response as ChatCompletion | SdkStream<ChatCompletionChunk>;
    }
    const result:any = response;
    return result.data as ChatCompletion | SdkStream<ChatCompletionChunk>;
  }

}
