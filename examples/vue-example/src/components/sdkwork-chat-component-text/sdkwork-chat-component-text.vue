<template>
  <div class="sdkwork-chat">
    ...
    <div class="chat-header">
      <slot name="header">
        <!-- 默认使用vant nav-bar -->
        <van-nav-bar :title="receiver?.nickname || '聊天对象'" :left-arrow="showBackButton" @click-left="handleBack"
    ...
    <!-- 消息列表区域 -->
    <div class="chat-body">
      <sdkwork-chat-message-list ref="messageListRef" :messages="processedMessages" :current-user-id="currentUser.id"
        :current-user="currentUser" :receiver="receiver" :show-avatar="showAvatar"
        :conversation-id="conversationId" @load-more="handleLoadMore" @message-click="handleMessageClick" />
    </div>

    <!-- 消息输入区域 -->
    <div class="chat-footer">
      <sdkwork-chat-input :placeholder="config?.placeholder || '输入消息...'" :max-length="config?.maxLength || 1000"
        :support-file="config?.enableFileUpload" :support-image="config?.enableFileUpload" :support-voice="true"
        @send="handleSendMessage" @upload-file="handleUploadFile" @keydown="handleKeydown" @focus="handleFocus"/>
    </div>
  </div>
</template>
...
import { Message } from 'sdkwork-sdk-api-typescript' 
...
const processedMessages = computed(() => {
  return messageStore.messages.map(msg => ({
    ...msg,
    isOwn: msg.senderId === props.currentUser.id
  }))
})
...
const handleLoadMore = async () => {
  try {
    if (props.conversationId) {
      // 调用message store的loadMoreMessages方法
      await messageStore.loadMoreMessages({
        conversationId: props.conversationId,
        page: 1, // 这里需要store维护分页状态
        size: 20
      })
      emit('load-more')
    }
...
const handleMessageReceived = (event: CustomEvent) => {
  ...
  if (type === 'MESSAGE_RECEIVED' || type === 'MESSAGE_STREAM_UPDATE') {
    // 接收到新消息或流式更新后滚动到底部
    nextTick(() => {
      messageListRef.value?.scrollToBottom()
    })
...
onMounted(() => {
  // 添加消息事件监听
  window.addEventListener('sdk:message', handleMessageReceived as EventListener)
  console.log('文本聊天组件已挂载并添加消息监听')
})

// 组件卸载时移除消息事件监听
onUnmounted(() => {
  // 移除消息事件监听
  window.removeEventListener('sdk:message', handleMessageReceived as EventListener)
  console.log('文本聊天组件已卸载并移除消息监听')
}) 
</script>
