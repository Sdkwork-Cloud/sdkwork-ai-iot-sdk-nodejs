...
import VoiceChatSubtitles from './components/voice-chat-subtitles/voice-chat-subtitles.vue'  
...
// 使用Agent状态管理
const { speakState, setSpeakState } = useAgent()
  ...
  if (isOn) {
    try {
      ...
      await startRecordStream({
        onData: async (audioData: ArrayBuffer) => {
          // 使用chatStore发送音频数据
          try {
            await chatStore.sendAudio(audioData)
          } catch (error) {
...
const setupAudioStreamListener = () => {
  ...
  const handleAudioStream = (event: any) => {
    if (event.detail && event.detail.type === 'AUDIO_STREAM_RECEIVED') {
      console.log('接收到音频流数据:', event.detail.audio)
      // 这里可以添加音频播放逻辑
  ...
  const handleMessageReceived = (event: any) => {
    if (event.detail && event.detail.type === 'MESSAGE_RECEIVED') {
      console.log('接收到消息:', event.detail.message)
      // 更新对话文本
...
onMounted(async () => {
  ...
  // 启动通话计时器
  const durationInterval = setInterval(updateCallDuration, 1000)
