 ...
 * Chat API接口实现
 */
export class ChatManager extends BaseSdkManager<SdkClient> {
  ...
  try {
    ...
    const response = await this._client.chat.stop(queryParams, options);
  ...
  try {
    ...
    const response = await this._client.chat.create(data, queryParams, options);
    if ( data.stream ) {
