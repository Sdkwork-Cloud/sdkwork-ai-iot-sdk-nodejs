...
export class FilesApi extends BaseSdkApi {
  ...
  try {
    // 构建请求URL
    const url = `${this.getBasePath(options)}/disk/files/${fileId}`;
  ...
  try {
    const url = `${this.getBasePath(options)}/disk/files`;

    // 构建请求参数
    const requestOptions: SdkRequestOptions = {
      ...options,
      url: `${this.getBasePath(options)}/disk/files`,
      method: 'POST',
  ...
  try {
    // 构建请求URL
    const url = `${this.getBasePath(options)}/disk/files/${fileId}/content`;
